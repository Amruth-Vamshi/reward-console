...
const OfferRedemptionRulesForm = Form.create<IProps>({ name: 'offer_redemption_rule' })(
  class OfferRedemptionRulesForm extends React.Component<IProps, {}> {
    render() {
      const { cappingData, wrappedComponentRef, form, formValues, offerType, cappingValue } = this.props;
      ...
      return (
        <Form {...formItemLayout} className='offerRedemptionForm' style={{ padding: '20px 50px' }} ref={wrappedComponentRef} layout="vertical">
          ...
          <Form.Item className="textPaddingTop"
            ...
            <span> Defined as maximum number of times an offer can be used collectively by target segment </span>
          </Form.Item>

          <Form.Item style={{ display: 'inline-block', width: 'calc(35% - 12px)' }} label="Usage Limit At Customer Level" >
            {getFieldDecorator('redemption_usage_limit_at_customer', {
              initialValue: `${Object.keys(formValues).length != 0 ? formValues.redemption_usage_limit_at_customer : ''}`,
            })(<Input type="number" />)}
          </Form.Item>
          <Form.Item className="textPaddingTop"
            style={{ display: 'inline-block', width: 'calc(65% - 12px)' }} >
          ...
          <Form.Item style={{ display: 'inline-block', width: 'calc(35% - 12px)' }} label="Time Limit" >
            {getFieldDecorator('redemption_time_limit', {
              initialValue: `${Object.keys(formValues).length != 0 ? formValues.redemption_time_limit : ''}`,
            })
              (<Input
                type="number" min={0}
                addonAfter={
                  <Select disabled getPopupContainer={(triggerNode: any) => triggerNode.parentNode} defaultValue="/day"
          ...
          <Form.Item style={{ display: 'inline-block', width: 'calc(35% - 12px)' }} label="Minimum SKU">
          ...
          <Form.Item
            ...
            style={{ display: 'inline-block', width: 'calc(65% - 12px)' }}
          >
            <span>Minimum units of Product to be present in the bill for offer to be applicable</span>
          </Form.Item>
						...
						<Form.Item style={{ display: 'inline-block', width: 'calc(35% - 12px)' }} label="Type">
							{getFieldDecorator('cappingType', {
								...
								<Select placeholder="Select a type" //onChange={this.handleSelectChange}
									...
									{offerType == "FREE_ITEMS_FROM_LIST" && <Option value="redemption_cap_no_of_items">No. of Items</Option>}
          ...
          {cappingValue ? <span>
            ...
            <Form.Item style={{ display: 'inline-block', width: 'calc(35% - 12px)' }} label="Type">
              {getFieldDecorator('cappingType', {
                ...
                <Select disabled placeholder="Select a type" //onChange={this.handleSelectChange}
