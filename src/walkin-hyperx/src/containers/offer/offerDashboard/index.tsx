...
export interface IAppProps
  extends RouteChildrenProps<any>,
  ...
  dataFormatter = data => {
    ...
    offerEligibilityRule &&
      offerEligibilityRule.ruleConfiguration.rules.map((rule: any) => {
        if (rule.attributeName.includes("product_"))
          products.push({
            type: rule.attributeName.replace("product_", ""),
            values: rule.attributeValue
          });
        else if (rule.attributeName.includes("location_"))
          location.push({
            type: rule.attributeName.replace("location_", ""),
            values: rule.attributeValue
          });
    ...
    rewardRedemptionRule &&
      rewardRedemptionRule.ruleConfiguration.rules.map((rule: any) => {
        if (rule.attributeName.includes("redemption_cap"))
          cappingType = rule.attributeName;
        redemption[rule.attributeName.replace("redemption_", "")] =
          rule.attributeValue;
      });
                ...
                <div className="gx-card">
                  <div style={{ padding: 35 }} className="gx-card-body">
                    <Row type="flex" justify="space-between" align="bottom">
                      <Col style={{ fontSize: 22, color: "black" }} span={18}>
                        {" "}
                        {name}{" "}
                      </Col>
                      <Col
                        ...
                        span={6}
                      >
                        <Button disabled style={{ marginBottom: 0 }}>
                          Edit
                        </Button>
                      </Col>
                    </Row>
                    ...
                    <Row>
                      ...
                      <Col {...wrapperCol}>
                        ...
                        -{" "}
                        {reward[offerType] ? reward[offerType].toString() : ""}
                        {fieldConvert(
                          offerTypeData,
                          offerType,
                          "value",
                    ...
                    <Row>
                      ...
                      <Col {...wrapperCol}>
                        {" "}
                        {redemption.usage_limit
                          ? `${redemption.usage_limit}  ${
                              redemption.usage_limit == 1 ? "time" : "times"
                            }`
                          : "  --"}
                      </Col>
                    </Row>
                    <Row>
                      ...
                      <Col {...wrapperCol}>
                        {" "}
                        {redemption.usage_limit_at_customer
                          ? `${redemption.usage_limit_at_customer} ${
                              redemption.usage_limit_at_customer == 1
                                ? "time"
                                : "times"
                            }`
                          : "  --"}
                    ...
                    {cappingType != "" ? (
                      <Row>
                        ...
                        <Col {...wrapperCol}>
                          {" "}
                          {redemption[cappingType.replace("redemption_", "")]
                            ? `${
                                redemption[
                                  cappingType.replace("redemption_", "")
                                ]
                              }`
                            : "  --"}
                        </Col>
